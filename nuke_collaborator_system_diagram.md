...
*   **Worker Process Shards**: There are `NUKE_WORKERS` long-lived worker subprocesses (default 4 ≈ host cores), and each active session group is **pinned to exactly one** of them — deterministically by `w{group_id % num_workers}` for unassigned groups, with the binding persisted in `groups.assigned_worker_id` (CELL-15; default `NULL` = unassigned, which spreads new groups instead of stacking them all on `w0`). So a worker serves *many* groups, but any one group only ever runs inside a single worker — which is precisely what lets a process-local per-group lock serialize that group's runs. This isolates memory leaks and provides basic security boundaries between projects. Workers use `asyncio` to execute agentic loops and tool calls.
...
*   **Per-group run serialization & interrupt-restart (`core/bg.py`, `runtime/dispatch.py`)**: because each group is pinned to one worker, a single process-local `asyncio.Lock` per group (`group_run_lock`) serializes that group's bot runs FIFO — back-to-back messages no longer fire-and-forget overlapping runs that interleave/clobber each other or drop a reply; a queued run reloads the latest history when its turn comes. A human free-chat follow-up first `abort_group`s the in-flight run, then starts a fresh one against the newest message (workflow-stage / non-human sends do **not** interrupt). The lock guarantees the cancelled run fully releases before the new one acquires — clean hand-off, no deadlock.

### L. IPC & Bus
*   **IPC (`runtime/ipc/`)**: length-framed JSON (4-byte big-endian header) over UDS/pipes (`transport_unix` / `transport_win`). `protocol.py` frame types group by plane:
    *   *Registration*: `HELLO` (worker / collector self-announces its `worker_id`).
    *   *Downstream (Supervisor → Worker)*: `USER_MESSAGE`, `ABORT`, `PERMISSION_RESPONSE`, `CONFIRM`, `START_WORKFLOW` / `WORKFLOW_NEXT` / `WORKFLOW_END`, `WAKE_TRIGGER`, `QUERY` / `MUTATE`, `RELEASE_LEASE`. (The permission *request* travels the other way — wrapped in a `BROADCAST` bus event — so only the response is a downstream frame.)
    *   *Upstream (Worker → Supervisor)*: `BROADCAST` (wraps any bus event for browser fan-out), `UNREAD_DELTA`, `STATS_REPORT` (DFT-057), `LEASE_RELEASED`.
    *   *MCP bus (relayed through the Supervisor)*: `MCP_CALL` / `MCP_RESULT` / `MCP_SCHEMAS`, `MCP_AUTH_START`, `MCP_OAUTH_CALLBACK`.
*   **Group-domain data proxy (`QUERY` / `MUTATE`, `runtime/query_dispatch.py`)**: a group's SQLite DB is owned *exclusively* by the worker holding that group's lease, so the Supervisor never opens it directly. Group-domain reads (history, reactions) and writes (reactions / pins / edits / deletes) are sent downstream as `QUERY` / `MUTATE`; the owning worker hits the group DB and replies with a correlated `query_result` bus event (writes additionally broadcast the update). Only central-domain data (users / groups / members / unread) stays Supervisor-side — the runtime corollary of the schema split (§N) and single-writer channel (§O).
*   **CELL-18 lease handoff (`runtime/supervisor.py`)**: to move a group between workers (e.g. the routing rebalance in §A) without split-brain on `group.lock`, the Supervisor sends `RELEASE_LEASE` to the current owner, waits for the `LEASE_RELEASED` ACK (group state flushed + lock released), then routes the group to the new worker — a clean transfer instead of two workers racing for the same DB.
