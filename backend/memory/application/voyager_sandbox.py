            ...
            if verify_fn is not None:
                ...
                if rollback_fn is not None:
                    try:
                        ...
                        return SkillRunResult(False, tuple(outputs), plan.verification, True, False)
                    except Exception:
                        logger.exception("skill rollback failed after verification failure")
                        return SkillRunResult(False, tuple(outputs), plan.verification, True, True)
                return SkillRunResult(False, tuple(outputs), plan.verification)
            return SkillRunResult(True, tuple(outputs), plan.verification)
        except Exception:
            if outputs and rollback_fn is not None:
                try:
                    rollback_result = rollback_fn(tuple(outputs))
                    if inspect.isawaitable(rollback_result):
                        await rollback_result
                except Exception:
                    logger.exception("skill rollback failed after execution exception")
            raise
