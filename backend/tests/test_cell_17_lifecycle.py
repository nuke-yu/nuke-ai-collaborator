...
class TestCell17Lifecycle(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_lru_eviction_skips_group_with_active_tasks(self):
        lm = LifecycleManager(max_groups=2)
        await lm.hydrate(1)
        await lm.hydrate(2)
    ...
    async def test_evict_still_closes_writer_and_releases_lock_if_abort_fails(self):
        lm = LifecycleManager()
        mock_lock = MagicMock()
        lm._locks[1] = mock_lock

        with patch("core.bg.abort_group", side_effect=RuntimeError("abort failed")), \
