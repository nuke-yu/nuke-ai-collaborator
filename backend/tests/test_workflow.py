...
class TestRunnerBroadcast(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_run_unit_logs_when_temp_promotion_notice_fails(self):
        from core import runner
        from executors.base import ExecutionResult
        from core.orchestration.base import OrchestratorStep, WorkUnit
        ...
        class StubOrch:
            def observe(self, gid, bid, resp, signals=None):
                return OrchestratorStep(done=False, next_units=[], announcements=[])
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            orig_ws = _const.WORKSPACE_ROOT
            _const.WORKSPACE_ROOT = root
            try:
                group_dir = layout.group_dir(1)
                (group_dir / "worktrees").mkdir(parents=True, exist_ok=True)

                async def fake_create_worktree(group_id, ticket_id, base_ref="main"):
                    wt = group_dir / "worktrees" / f"task_{ticket_id}"
                    wt.mkdir(parents=True, exist_ok=True)
                    return wt

                def fake_use_worktree(group_id, worktree_path):
                    return nullcontext()

                async def fake_post_system_msg(*_args, **_kwargs):
                    raise RuntimeError("notify failed")

                unit = WorkUnit(bot=_bot_entry(), executor_id="fake", trigger_msg="go")

                with patch.object(runner, "get_members", new=AsyncMock(return_value=[_bot_entry()])), \
                     patch.object(runner, "get_messages", new=AsyncMock(return_value=[])), \
                     patch.object(runner, "get_db", return_value=fake_db_cm), \
                     patch.object(runner, "global_db", return_value=fake_db_cm), \
                     patch.object(runner.exec_registry, "get", return_value=FakeExec()), \
                     patch.object(runner, "apply_step", new=AsyncMock()), \
                     patch("workspace.git_worktree.create_worktree", new=fake_create_worktree), \
                     patch("workspace.git_worktree.use_worktree", new=fake_use_worktree), \
                     patch("workspace.git_worktree.promote_worktree", side_effect=RuntimeError("merge failed")), \
                     patch("core.runner._post_system_msg", new=fake_post_system_msg), \
                     self.assertLogs("core.runner", level="WARNING") as logs:
                    await runner.run_unit(1, unit, StubOrch())

                self.assertTrue(any("failed to post immediate promotion failure message" in line for line in logs.output))
            finally:
                _const.WORKSPACE_ROOT = orig_ws
