    ...
    original_config = {
        "mcpServers": {
            "test-server": {
                "command": "node",
                "args": ["test.js"],
                "env": {"API_KEY": "supersecret12345", "DEBUG": "true"},
...
def test_put_logs_when_etag_computation_fails(client, temp_mcp_config, caplog):
    ...
    def fake_read_bytes(self, *args, **kwargs):
        if str(self) == temp_mcp_config:
            raise OSError("etag failed")
        return orig_read_bytes(self, *args, **kwargs)

    with patch("pathlib.Path.read_bytes", new=fake_read_bytes), \
         caplog.at_level(logging.WARNING, logger="api.config"):
        response = client.put("/api/config/mcp", json=new_config)

    assert response.status_code == 200
    assert response.json()["mcpServers"]["s"]["command"] == "node"
    assert any("failed to compute ETag" in record.message for record in caplog.records)
