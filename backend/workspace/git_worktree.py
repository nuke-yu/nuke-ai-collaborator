...
def use_worktree(worktree_path: Path):
    """Context manager to temporarily bind VFS and shell paths to a worktree."""
    token = current_workspace_path.set(worktree_path)
    try:
        yield
    finally:
...
async def _run_git_cmd(cwd: Path, *args: str) -> str:
        ...
        for item in shared_dir.iterdir():
            ...
            if not dest.exists():
                try:
                    ...
                    # Using relative symlink to be robust to workspace relocations
                    ...
                    log.warning(f"Failed to symlink shared resource {item.name}: {e}")
    except Exception as e:
        log.warning(f"Failed to list shared resources for symlinking: {e}")
                ...
                if item.is_dir():
                    ...
                    if name in dependency_names:
                        ...
                        if not target.exists():
                            try:
                                # Use absolute path for dependency folders since they might be large and outside relative depth
                                target.symlink_to(item.resolve())
                                log.info(f"Symlinked dependency: {target} -> {item}")
                            except Exception as e:
                                log.warning(f"Failed to symlink dependency {name} -> {item}: {e}")
                    elif name not in {".git", "worktrees", "runs", ".history"}:
                        _scan_and_link(item, dest / name, depth - 1)
        except Exception as e:
            log.warning(f"Error scanning dependencies in {src}: {e}")
...
async def create_worktree(group_id: int, task_id: str, base_ref: str = "main") -> Path:
    ...
    # 1. Initialize git repo if not exists (for local testing/resilience)
    if not (shared_workspace / ".git").exists():
        ...
        readme = shared_workspace / "README.md"
        if not readme.exists():
            readme.write_text("# Shared Workspace\n", encoding="utf-8")
        await _run_git_cmd(shared_workspace, "add", "README.md")
        await _run_git_cmd(shared_workspace, "commit", "-m", "Initial commit")
        try:
    ...
    # 3. Check and delete task branch if it exists (so we start fresh)
    try:
    ...
    # 4. Link shared coordination resources
    link_shared_resources(shared_dir, worktree_dir)

    # 5. Link heavy dependencies
...
async def remove_worktree(group_id: int, task_id: str):
    """Force remove a git worktree and delete its branch."""
    group_dir = layout.group_dir(group_id)
    ...
    if (shared_workspace / ".git").exists() and worktree_workspace.exists():
        try:
            ...
            log.warning(f"git worktree remove failed: {e}")
    ...
    if worktree_dir.exists():
        try:
            ...
            log.warning(f"shutil.rmtree failed for {worktree_dir}: {e}")
...
async def promote_worktree(group_id: int, task_id: str, target_branch: str = "main"):
    ...
    if worktree_workspace.exists():
        try:
            ...
            log.warning(f"Failed to auto-commit uncommitted changes in worktree {worktree_workspace}: {e}")
    ...
    # Clean up the worktree directory and git branch
