...
async def migration_050(db):
    """Add canonical provenance and temporal relations between Memory records."""
    cur = await db.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='memory_records'"
    )
    if await cur.fetchone() is None:
    ...
    await db.execute(
        ...
        ON memory_relations(group_id,from_record_id,status,relation_type)"""
    )
    await db.execute(
        """CREATE INDEX IF NOT EXISTS idx_memory_relations_to
        ON memory_relations(group_id,to_record_id,status,relation_type)"""
    )
    await db.commit()
...
async def migration_058(db):
    """Add an exact request-level model usage ledger."""
    cur = await db.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='session_events'"
    )
    if await cur.fetchone() is None:
    ...
    await db.execute("""CREATE INDEX IF NOT EXISTS idx_model_usage_session
        ON model_usage_ledger(session_id,request_ordinal)""")
    await db.execute("""CREATE INDEX IF NOT EXISTS idx_model_usage_status
        ON model_usage_ledger(status,started_at)""")
    await db.commit()
...
async def migration_063(db):
    """Scope Case run identity by Group instead of global run_id."""
    cur = await db.execute(
        "SELECT 1 FROM sqlite_master WHERE type='table' AND name='agent_cases'"
    )
    if await cur.fetchone() is None:
        return
...
async def run_migrations(db) -> None:
    ...
    await _run_migrations_impl(db, MIGRATIONS)
