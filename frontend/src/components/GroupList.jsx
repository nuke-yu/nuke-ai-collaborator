  ...
  return (
    ...
    <div className={`w-full md:w-56 bg-gray-900 text-white flex flex-col flex-shrink-0 ${className}`}>
      ...
      <div className="flex items-center justify-between px-3 pt-3 pb-1 flex-shrink-0">
        <span className="text-xs font-semibold text-gray-500 uppercase tracking-wider">{t(K.sidebar.groups)}</span>
        <button onClick={() => setCreating(true)} className="text-gray-500 hover:text-white text-lg leading-none transition-colors" title={t(K.sidebar.newGroup)}>+</button>
      </div>
      ...
      {creating && (
        <div className="px-2 pb-2 flex-shrink-0">
          <input
            ...
            placeholder={t(K.sidebar.groupNamePlaceholder)}
            value={newName}
          ...
          <div className="flex gap-1 mt-1">
            <button onClick={handleCreate} className="flex-1 bg-indigo-600 hover:bg-indigo-500 text-white text-xs rounded py-1 transition-colors">{t(K.sidebar.create)}</button>
            <button onClick={() => { setCreating(false); setNewName('') }} className="flex-1 bg-gray-700 hover:bg-gray-600 text-gray-300 text-xs rounded py-1 transition-colors">{t(K.sidebar.cancel)}</button>
          </div>
        </div>
      )}
          ...
          <div key={g.id}>
              ...
              {confirmDeleteGroupId === g.id ? (
                <div className="flex items-center gap-1 pr-1 flex-shrink-0">
                  <button onClick={() => { onDeleteGroup?.(g.id); setConfirmDeleteGroupId(null) }}
                    className="text-xs bg-red-600 hover:bg-red-500 text-white px-1.5 py-0.5 rounded transition-colors">{t(K.sidebar.delete)}</button>
                  <button onClick={() => setConfirmDeleteGroupId(null)}
                    className="text-xs text-gray-400 hover:text-white px-1 py-0.5 rounded transition-colors">{t(K.sidebar.cancel)}</button>
                </div>
              ) : (
                <button
            ...
            {expandedGroups.has(g.id) && (
              <div className="mt-0.5 mb-1">
                {(membersCache[g.id] || []).map((m) => (
                  <div key={m.id} className="group flex items-center gap-1.5 pl-5 pr-2 py-0.5 rounded hover:bg-gray-800/50 transition-colors">
                    ...
                    {m.type === 'bot' && (
                      <>
                        {skillDraftBots.has(m.id)
                          ? <span className="group-hover:hidden text-xs text-yellow-400 flex-shrink-0" title={t(K.sidebar.skillPending)}>⚡</span>
                          : <span className="group-hover:hidden text-xs text-gray-600 flex-shrink-0">🤖</span>
                        }
                        <button
                        ...
                        <button
                          ...
                          title={skillDraftBots.has(m.id) ? t(K.sidebar.skillPending) : t(K.sidebar.workspaceFiles)}
                        >
                    ...
                    {m.id === currentMemberId && (
                      <>
                        {m.auto_reply && <span className="group-hover:hidden text-xs text-indigo-600 flex-shrink-0" title={t(K.sidebar.autoReplyEnabled)}>↩</span>}
                        <button
                          ...
                          title={t(K.sidebar.setAutoReply)}
                        >↩</button>
                      </>
                    )}
                ...
                >
                  <span>+</span>
                  <span>{t(K.sidebar.addMember)}</span>
                </button>
              </div>
            )}
          </div>
