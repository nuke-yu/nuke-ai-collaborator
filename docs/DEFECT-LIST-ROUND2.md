...
- **文件**：`executors/plugins/workspace_tools.py:550-558`
- **问题**：之前的匹配逻辑会导致 `head -80` 这种命令被误拦截。
- **影响**：命令语义被改坏，难以排查。
- **状态**：✅ 已修复（引入了更严格的正则负向断言 `(?<![\\d\\-])80(?![\\d])`，消除了误匹配）
...
- **状态**：✅ 鉴权层已做 · 深层数据隔离**按设计暂缓**（内网/可信场景）
- **文件**：`main.py`（WS 握手 token 校验）· `core/auth.py`（JWT）· REST 路由挂 `Depends(get_current_user)`
- **影响**：对外化前的地基缺失；任何人可冒任何身份。
- **范围与决定（2026-06-02 复审）**：本项目是**公司内部 / 可信小圈子 / 不对外网**的多 Bot 协作工作区，不是公网多租户 SaaS。结论：**token（登录）= 访问边界，够用**；REST 不按 user 切数据、WS 不绑 member 归属，均为**有意为之**，深层多租户隔离暂缓。
- **⚠️ 反面教训（别再翻案）**：`members.user_id` 列是为未来预留的，**当前没有任何代码给它赋值**。曾两次有人把「`members.user_id == token uid`」的严格 WS 校验加进来当成「修复身份冒用」——但因 user_id 恒为 NULL，**它会拒绝每一个连接、把实时聊天整体弄断**。现行 WS 鉴权刻意为「有效 token + 校验 member 属于该群」。**如果将来真要对外**，才需要做完整两步：① `add_member` 写 `user_id` + 把列补进 legacy schema/迁移；② 再加严格校验 + REST 按 user 收口。
