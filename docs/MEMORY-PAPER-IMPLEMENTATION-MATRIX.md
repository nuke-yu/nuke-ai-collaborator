...
> 状态口径：`已在线` 表示默认生产用例确实调用；`部分在线` 表示存在生产子链路但不是完整能力；`已实现` 仅表示适配器/算法和测试存在；`适配器` 不等于生产能力。详细状态见 [Memory Capability Wiring Status](decisions/MEMORY-CAPABILITY-STATUS.md)。
...
| 时序事实与关系失效 | **ZEP/Graphiti**，`2501.13956-Graphiti.pdf` §2–§3 | 保留事实历史、有效时间和 supersede 关系，支持 as-of 查询 | `graphiti_temporal_engine.py`；`CanonicalRelationService`；`MemoryComposition.temporal_graph` | **已实现/已组合，默认热召回未启用** | 适配器支持多跳和 hybrid API，但当前不能宣称所有热查询都经过 Graph lane；持久化和线上规模需单独验证 |
| 多路召回融合 | **RRF**（Graphiti/ZEP 引用） | lexical、vector、graph、cluster 分数不在同一尺度时仍能稳定融合排序 | `memory/adapters/algorithms/hybrid_rerank_engine.py` | **已实现，默认路径部分使用** | 必须把是否调用 reranker 作为可观测配置，不把适配器存在写成所有召回已融合 |
| 结果去冗余 | **MMR**（Graphiti/ZEP 引用） | 在相关性和多样性之间取平衡，减少重复记忆占用上下文 | `memory/adapters/algorithms/hybrid_rerank_engine.py` | **已实现，非全局默认** | MMR 只负责排序，不负责权限、事实有效性或最终预算 |
