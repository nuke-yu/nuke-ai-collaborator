...
| Channel Adapter | TRANSITION | C0–C8 基础模块和两轮 Review 阻断修复已提交；C6/C7 已达到本地 failure-tested，包含持久化 projection 补偿、instance migration/quarantine、source lineage、双层 dead-letter 和可恢复健康指标；C8 已接入配置驱动的 ProcessClient 注册与 active Binding fail-fast。真实平台 Connector executable、平台 E2E 和 go/no-go 演练仍未完成。 |
...
#### Phase 6：Channel Adapter（核心链路已 failure-tested，真实平台 Connector 待完成）

Channel 先作为独立模块运行，只有通过显式 Channel-Group Bridge 和 active Binding 后，才作为 Group Integration Member 与 Group 双向通信。当前 Workflow → durable projection queue → Group transactional outbox → Supervisor Relay → Channel delivery outbox → Dispatcher → configured ProcessClient 已接通，并覆盖补偿重放、历史实例隔离、lineage、poison dead-letter 和健康恢复。active Binding 缺 Connector 配置会启动失败。剩余阻断项是选择并交付一个真实平台 Connector executable、外部用户授权、附件 Artifact、平台限流/重放 E2E、部署 smoke test 和 go/no-go 演练；云平台级隔离与多租户编排后置。
...
截至本轮复核，后端全量测试为 `2522 passed, 2 skipped, 56 warnings, 40 subtests passed`。全量回归在允许本地 Unix/TCP socket 的环境执行；沙箱环境会因禁止 bind 而产生非代码级 PermissionError。本轮之后，Channel 的 Group 事务 Outbox producer、Supervisor Relay 生命周期、Pause/Resume、死信 replay、SLO 指标、Process Server、最小子进程环境、Secret Resolver 和当前范围 Docker/Compose 测试部署已具备；当前剩余工作集中在真实渠道端到端接入、外部用户授权映射和附件 Artifact 化。云平台隔离与多租户编排后置。
